...
@section('content')
    ...
    <div style="display: flex;float: left;">
        ...
        <div style="margin-left: 100px">
            <form action="" method="GET">
                @csrf
                <select class="form-control-sm" style="height: 38px;" name="trang_thai" id="">
                    <option class="form-control" value="">----chọn trạng thái----</option>
                    <option class="form-control" value="2">Đang chờ xác nhận</option>
                    <option class="form-control" value="3">Đã các nhận</option>
                    <option class="form-control" value="4">Đã thanh toán</option>
                    <option class="form-control" value="5">Từ chối yêu cầu</option>
                </select>
                <button class="btn btn-outline-info"><i class="fas fa-check-circle"></i></button>
            </form>
        </div>
    </div>
    <br>
        ...
        <table class="table table-bordered">
            <thead>
                <tr>
                    ...
                    <th scope="col">Tiền chênh lệch </th>
                        ...
                        <td>
                            @if ($item->status == 0 || $item->status == 1)
                                <form action="{{ route('route_BE_Admin_updateStatus_doilop', $item->id) }}" method="POST">
                                    @csrf
                                    @method('PUT')
                                    <input hidden type="text" name="id_lopcu" value="{{ $id_lopcu }}"
                                        id="">
                                    <input hidden type="text" name="id_lopmoi" value="{{ $item->lop_id }}"
                                        id="">
                                    <input type="text" name="user_id" value="{{ $item->id_user }}" hidden
                                        id="">
                                    <select {{ $item->status == 1 ? 'disabled' : '' }}
                                        ...
                                        <option {{ $item->status == 1 ? 'selected' : '' }} value="1">Đã xác nhận
                                        </option>
                            ...
                            @if ($item->status == 2 || $item->status == 3 || $item->status == 4 || $item->status == 5)
                                <form action="{{ route('route_BE_Admin_updateStatus_doilop', $item->id) }}" method="POST">
                                    ...
                                    <select {{ $item->status == 4 ? 'disabled' : '' }}
                                        ...
                                        <option {{ $item->status == 3 ? 'selected' : '' }} value="3">Đã xác nhận
                                        </option>
                                    ...
                                    <button {{ $item->status == 4 ? 'disabled' : '' }}
                                        ...
                                        style="height: 30px;" class="btn btn-outline-info" type="submit"><i
                                            style="" class="fas fa-check"></i></button>
                                </form>
                            @endif
                        </td>
                    </tr>
    ...
    @else
        <table class="table table-bordered">
            ...
            <tbody>
                @foreach ($loc_trang_thai as $key => $item)
                    <tr>
                        <td><input class="checkitem" type="checkbox" name="id[]" value="{{ $item->id }}" /></td>
                        <td> {{ $item->id }}</td>
                        <td> {{ $item->user_id }}</td>
                        ...
                        <td>
                            @if ($item->status == 0 || $item->status == 1)
                                <form action="{{ route('route_BE_Admin_updateStatus_doilop', $item->id) }}"
                                    method="POST">
                                    @csrf
                                    @method('PUT')
                                    <input hidden type="text" name="id_lopcu" value="{{ $id_lopcu }}"
                                    ...
                                    <select {{ $item->status == 1 ? 'disabled' : '' }}
                                        {{ $item->status == 5 ? 'disabled' : '' }} style="height: 30px" name="status"
                                        id="">
                                        <option {{ $item->status == 0 ? 'selected' : '' }} value="0">Đang chờ xác
                                            nhận
                                        </option>
                                        <option {{ $item->status == 1 ? 'selected' : '' }} value="1">Đã xác nhận
                                        </option>
